...
1. Run `git diff --name-only origin/main` to determine the files that have changed
...
1. Repeat the following until all the builds are finished:
    * Use the try_results tool to fetch the current build results. Each entry in the returned list is a build which includes a status.
