...
**Use code search.**
You can search for code using the `cs` command line tool.
Note that results will only contain landed changes - any unsubmitted changes that you or the user have made in the current checkout will not be reflected.
...
- The git:chromium/chromium/src@main is mandatory.
- Control the amount of output with max_num_results and context (which is the number of LOC displayed around a match).

Example:
```bash
cs --max_num_results=10 --context=5 -- git:chromium/chromium/src@main case:y -f:/out/ -f:/test/ form_field_data f:renderer 
```

Never attempt to search the root chromium/src folder locally. This takes far too much
time and requires using a subdirectory of chromium/src for searching. Prefer to use `cs`
when possible
...
1. Run `git diff origin/main...HEAD` to determine the what has changed.
