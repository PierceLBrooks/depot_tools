...
class BuildbucketTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_build_status_success(self, mock_subprocess_run):
        ...
        status = await buildbucket.get_build_status(self.mock_context, build_id)
    ...
    @patch('subprocess.run')
    async def test_search_builds_success(self, mock_subprocess_run):
        ...
        status = await buildbucket.search_builds(self.mock_context,
                                                 builder_name, builder_bucket,
                                                 builder_project)
    ...
    @patch('subprocess.run')
    async def test_search_builds_exception(self, mock_subprocess_run):
        builder_name = 'test_builder'
        mock_subprocess_run.side_effect = Exception('PRPC call failed')

        result = await buildbucket.search_builds(self.mock_context,
                                                 builder_name)
    ...
    async def test_get_build_from_id_success(self, mock_subprocess_run):
        ...
        output = await buildbucket.get_build_from_id(self.mock_context,
                                                     build_id, fields)
    ...
    async def test_get_build_from_id_exception(self, mock_subprocess_run):
        ...
        result = await buildbucket.get_build_from_id(self.mock_context,
                                                     build_id, fields)
    ...
    @patch('subprocess.run')
    async def test_get_build_from_build_number_success(self,
                                                       mock_subprocess_run):
        build_number = 987
        builder_name = 'test_builder'
        builder_bucket = 'try'
