...
class GitClTest(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('subprocess.run')
    async def test_try_builder_results_exception(self, mock_subprocess_run):
        mock_subprocess_run.side_effect = Exception('git cl try-results failed')

        result = await git_cl.try_builder_results(self.mock_context,
                                                  self.checkout)

        self.assertIn('Exception calling prpc', result)
        self.assertIn('git cl try-results failed', result)
    ...
    @patch('subprocess.run')
    async def test_checkout_change_list_success(self, mock_subprocess_run):
        issue = 12345
        expected_output = f'Checked out issue {issue}'
        mock_subprocess_run.return_value = subprocess.CompletedProcess(
            args=[], returncode=0, stdout=expected_output, stderr='')

        output = await git_cl.checkout_change_list(self.mock_context,
                                                   self.checkout, issue)

        self.assertEqual(output, expected_output)
        expected_command = ["git", "cl", "checkout", str(issue)]
        mock_subprocess_run.assert_called_once_with(expected_command,
                                                    capture_output=True,
                                                    check=False,
                                                    text=True,
